...
export const ShareButtons = ({ url, titulo, preco }: ShareButtonsProps) => {
  const texto = `Confira este veículo: ${titulo} por ${formatPrice(preco)}`;
  ...
  const handleShare = (platform: string) => {
    ...
    switch (platform) {
      ...
      case 'telegram':
        ...
        break;
      case 'email':
        shareUrl = `mailto:?subject=${encodeURIComponent(titulo)}&body=${encodeURIComponent(
          texto + '\n\n' + url
        )}`;
        break;
    }
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        >
          ...
          <span className="text-xs text-gray-600">Copiar</span>
        </button>
      </div>
    </div>
  );
